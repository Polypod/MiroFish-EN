...
def reset_project(project_id: str):
    """
    Reset project status (used for rebuilding the graph).
    """
    project = ProjectManager.get_project(project_id)
...
def generate_ontology():
    """
    Endpoint 1: Upload files and analyze them to generate ontology definitions.

    Request type: multipart/form-data

    Parameters:
        files: Uploaded files (PDF/MD/TXT), supports multiple
    ...
    """
    try:
        logger.info("=== Start generating ontology definitions ===")
        ...
        logger.error(f"generate_ontology failed: {str(e)}")
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "Project ontology not generated yet. Call /ontology/generate first"
            }), 400
        
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is currently building. Do not submit repeatedly. Add force: true to rebuild",
