...
# Adding this prefix avoids Agent tool invocation, responds with text directly
INTERVIEW_PROMPT_PREFIX = "Based on your persona, all past memories and actions, reply directly with text without invoking any tools："


def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview questions, add prefix to avoid Agent tool invocation.
...
def get_graph_entities(graph_id: str):
    ...
    Returns only nodes that match predefined entity types (nodes with Labels not just Entity)
...
def create_simulation():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Project graph not built yet, please call /api/graph/build first"
            }), 400
...
def prepare_simulation():
    ...
    - If already prepared, directly return existing results
    ...
    Returns:
        {
            ...
            "data": {
                "simulation_id": "sim_xxxx",
                "task_id": "task_xxxx",           // Returned when new task created
                "status": "preparing|ready",
                "message": "Preparation task started|Preparation already complete",
                "already_prepared": true|false    // if preparation is complete
            }
        }
    """
    import threading
    ...
    try:
        ...
        if not force_regenerate:
            logger.debug(f"Check simulation {simulation_id} if preparation is complete...")
        ...
        try:
            ...
            reader = ZepEntityReader()
            # Quickly read entities (no edge information, just count)
            filtered_preview = reader.filter_defined_entities(
                graph_id=state.graph_id,
                defined_entity_types=entity_types_list,
                enrich_with_edges=False  # Don't get edge information, speed up
            )
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "Preparation task started, please use /api/simulation/prepare/status to check progress",
...
def get_prepare_status():
    ...
    2. Via simulation_id check if preparation is already complete
    ...
    try:
        ...
        if not task_id:
            if simulation_id:
                # simulation_id is provided but preparation is not complete
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
                        "status": "not_started",
                        "progress": 0,
                        "message": "Not yet started, please call /api/simulation/prepare to start",
                        "already_prepared": False
                    }
                })
            return jsonify({
        ...
        if not task:
            # Task does not exist, but if has simulation_id, check if already prepared
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation_history():
    ...
    For homepage historical project display, return a rich simulation list with project names and descriptions
    ...
    try:
        ...
        # Enhance simulation data, only read from Simulation files
        ...
        for sim in simulations:
            ...
            # Get run status (read actual user-set rounds from run_state.json)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get simulation Agent Profile in real-time (for viewing progress during generation)
    
    Different from /profiles endpoint：
    - Read files directly, not through SimulationManager
    - Suitable for real-time viewing during generation
    - Return additional metadata (such as file modification time, whether generation is in progress, etc)
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "platform": "reddit",
                "count": 15,
                "total_expected": 93,  // Expected total (if exists)
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Get simulation config in real-time (to view progress during generation)
    
    Different from /config endpoint：
    - Read files directly, not through SimulationManager
    - Suitable for real-time viewing during generation
    - Return additional metadata (such as file modification time, whether generation is in progress, etc)
    ...
    Request (JSON)：
        {
            ...
            "enable_graph_memory_update": false,   // optional: Whether to update Agent activities to Zep graph memory in real-time
            "force": false                         // optional: Force restart (will stop running simulation and clean logs)
        }

    About force parameter：
        - If enabled, will first stop and clean run logs if simulation is running or completed
        - Content to be cleaned includes: run_state.json, actions.jsonl, simulation.log, etc.
        - Will not clean config files (simulation_config.json) and profile files
        - Applicable to scenarios where simulation needs to be re-run

    About enable_graph_memory_update：
        - If enabled, all Agent activities in simulation (posting, commenting, liking, etc) will be updated to Zep graph in real-time
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is currently running. Please call /stop first, or use force=true to restart."
                            }), 400

                # In force mode, clean run logs
                if force:
                ...
                logger.info(f"Simulation {simulation_id} preparation completed, reset status to ready (previous status: {state.status.value})")
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Enabling graph memory updates requires a valid graph_id. Please ensure the project graph has been built."
                }), 400
...
def get_run_status(simulation_id: str):
    """
    Get simulation real-time status (for frontend polling)
...
def get_run_status_detail(simulation_id: str):
    """
    Get detailed simulation status (including all actions)
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "twitter_actions": [...],  # All actions from the Twitter platform
                "reddit_actions": [...]    # All actions from the Reddit platform
            }
        }
    """
    try:
        ...
        # Get actions grouped by platform
        ...
        # recent_actions only shows content from the latest current round across both platforms
...
def get_simulation_timeline(simulation_id: str):
    ...
    For frontend display of progress bar and timeline view
...
def interview_agent():
    ...
    Note: This feature requires simulation environment to be in running state (enter command-waiting mode after simulation cycle)
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "The simulation environment is not running or has been closed. Please ensure the simulation is complete and in command-wait mode."
            }), 400
        
        # Optimize prompt by adding a prefix to avoid Agent tool invocation
...
def interview_agents_batch():
    ...
    Request (JSON)：
        {
            ...
            "interviews": [                    // required，Interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What is your opinion on B?"  // If platform is not specified, use default value
                }
            ],
            "platform": "reddit",              // optional, default platform (overridden by each item's platform)
                                               // When not specified: dual-platform simulation interviews both platforms for each Agent simultaneously
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            # Validate platform in each item (if present)
            item_platform = interview.get('platform')
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"platform for interview list item {i+1} must be either 'twitter' or 'reddit'"
                }), 400
        ...
        # Optimize each interview prompt by adding a prefix to avoid Agent tool invocation
...
def interview_all_agents():
    """
    Global interview - interview all Agents with same question
    ...
    Request (JSON)：
        {
            "simulation_id": "sim_xxxx",            // required，Simulation ID
            "prompt": "What is your overall view on this matter?",  // required, Interview question (all agents use the same prompt)
    ...
    try:
        ...
        return jsonify({
            "success": False,
                "error": f"Timed out waiting for global interview responses: {str(e)}"
        }), 504
    ...
    Request (JSON)：
        {
            ...
            "limit": 100                   // optional, Number of records to return, default 100
        }
...
def close_simulation_env():
    ...
    Send close environment command to simulation to gracefully exit command-waiting mode。
    
    Note: This is different from /stop endpoint, /stop forces process termination，
    This endpoint allows simulation to gracefully close environment and exit。
