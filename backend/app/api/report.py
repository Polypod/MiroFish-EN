...
from ..services.report_manager import ReportManager, ReportStatus
...
def get_report_sections(report_id: str):
    """
    Get list of generated sections (split output by section)
    
    Frontend can poll this endpoint to get section content as it's generated, without waiting for the entire report to complete
...
def check_report_status(simulation_id: str):
    ...
    try:
        ...
        # Unlock interview only when report is completed
...
def get_agent_log(report_id: str):
    ...
    Fetch each step of the report generation process in real-time, including:
    ...
    try:
        ...
        logger.error(f"Failed to get Agentlog failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def stream_agent_log(report_id: str):
    """
    Get complete Agent logs (fetch all at once).
...
def get_console_log(report_id: str):
    """
    Get Report Agent's console output logs.

    Fetch console output (INFO, WARNING, etc.) during report generation in real-time.
    ...
    try:
        ...
        logger.error(f"Failed to get consolelog failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def stream_console_log(report_id: str):
    """
    Get complete console logs (fetch all at once).
