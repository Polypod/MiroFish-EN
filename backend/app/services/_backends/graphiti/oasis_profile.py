...
Convert entities in the Zep graph into Agent Profile format required by the OASIS simulation platform

Optimizations:
1. Enrich node information with secondary retrieval from Zep
2. Improve prompts to generate highly detailed personas
...
class OasisProfileGenerator:
    ...
    Convert entities in the Zep graph into Agent Profiles required by OASIS simulation
    
    Optimization features:
    1. Retrieve richer context via Zep graph search
    2. Generate highly detailed personas (basic info, career, personality traits, social-media behavior, etc.)
    3. Distinguish individual entities from abstract/group entities
    """
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Retrieve rich entity-related information with mixed Zep graph search
        
        Zep has no built-in mixed-search endpoint, so edges and nodes are searched separately and merged.
        Parallel requests are used to improve efficiency.
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Rich information retrieved from Zep mixed search
        """
        context_parts = []
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Check whether entity is an individual type"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES
    
    def _is_group_entity(self, entity_type: str) -> bool:
        """Check whether entity is a group/institution type"""
    ...
    def _generate_profile_with_llm(
        ...
        """
        Generate highly detailed personas using LLM
        
        Distinguish by entity type:
        - Individual entity: generate concrete personal profile
        ...
        # Retry multiple times until success or max attempts reached
        max_attempts = 3
        last_error = None
    ...
    def _fix_truncated_json(self, content: str) -> str:
        """Fix truncated JSON (output truncated by max_tokens limits)"""
        import re
        ...
        # Simple check: if the last char after a quote is not a comma/closing brace, string may be truncated
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 2. Try extracting JSON segment
        json_match = re.search(r'\{[\s\S]*\}', content)
        if json_match:
            json_str = json_match.group()
            
            # 3. Fix newline issues inside strings
            # Find all JSON string values and replace newlines in them
            def fix_string_newlines(match):
                s = match.group(0)
                # Replace real newlines in string with spaces
                s = s.replace('\n', ' ').replace('\r', ' ')
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in social-media user profiling. Generate detailed and realistic personas for public-opinion simulation, matching known real-world context as closely as possible. You must return valid JSON, and string values must not include unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social-media user persona for this entity, matching known real-world context as closely as possible.
...
2. persona: Detailed persona description (about 2000 words plain text), including:
   - Basic information (age, profession, education, location)
   - Background (important experiences, event relevance, social relationships)
   ...
   - Stance and viewpoints (attitude toward topics, potential triggers)
   - Unique traits (catchphrases, special experiences, hobbies)
   - Personal memory (critical persona part: event relation and existing actions/reactions)
...
- persona must be one coherent text paragraph
- Use English
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build a detailed persona prompt for group/institution entities."""
        
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"
        
        return f"""Generate a detailed social-media account profile for this organization/group entity, matching known real-world context as closely as possible.
...
1. bio: Official account bio (about 200 words, professional tone)
2. persona: Detailed account profile (about 2000 words plain text), including:
   - Organization basics (formal name, nature, founding background, main functions)
   ...
   - Content publishing patterns (content types, posting frequency, active time windows)
   - Stance handling (official stance on key topics, handling of controversy)
   - Special notes (represented group profile, operation habits)
   - Institutional memory (event relation and existing actions/reactions)
3. age: Fixed to 30 (virtual age for institutional account)
4. gender: Fixed to "other" (institutional account is non-personal)
...
8. interested_topics: Array of focus domains

Important:
- All field values must be strings or numbers; null is not allowed
- persona must be one coherent text paragraph, without newlines
- Use English
- age must be integer 30, and gender must be string "other"
- Institutional account voice must match identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Graph ID for richer context via Zep retrieval
            parallel_count: Parallel generation count, default 5
            realtime_output_path: Real-time output file path (if provided, write after each profile)
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                # Output generated persona in real time to console and logs
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        ...
        # Print to console only (avoid duplicates; logger no longer prints full content)
        print(output)
    ...
    def save_profiles(
        ...
        """
        Save profiles to file (choose correct format by platform).
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        - user_id: User ID (starts from 0 by CSV order)
        - name: Real user name
        ...
        - description: Short public bio (shown on profile page)
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Uses a format consistent with to_reddit_format() to ensure OASIS reads correctly.
        Must include user_id, which is key for OASIS agent_graph.get_agent() matching.
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                "user_id": profile.user_id if profile.user_id is not None else idx,  # Key: user_id is required
