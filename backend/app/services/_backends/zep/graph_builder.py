...
class GraphBuilderService:
    """
    Graph building service.
    Responsible for building the knowledge graph through Zep API calls.
    """
    ...
    def _build_graph_worker(
        ...
        try:
            ...
            # 5. Wait for Zep processing completion
            self.task_manager.update_task(
                task_id,
                progress=60,
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # This usage is required by Zep SDK; warning comes from dynamic class creation and is safe to ignore
    ...
    def add_text_batches(
        ...
        """Add text to graph in batches and return UUID list for all episodes."""
        episode_uuids = []
        total_chunks = len(chunks)
    ...
    def _wait_for_episodes(
        ...
        """Wait for all episodes to finish processing (by checking each episode processed status)."""
        ...
        if progress_callback:
            progress_callback(f"Start waiting for {total_episodes} text chunks to be processed...", 0)
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        ...
        # Create node map for node name lookup
        node_map = {}
        for node in nodes:
