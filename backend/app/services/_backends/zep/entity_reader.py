...
Reads nodes from a Zep graph and filters nodes that match predefined entity types.
"""
...
class EntityNode:
    ...
    def get_entity_type(self) -> Optional[str]:
        """Get entity type (excluding default labels like Entity)."""
...
class ZepEntityReader:
    ...
    2. Filter nodes that match predefined entity types (labels beyond just Entity)
    ...
    def _call_with_retry(
        ...
        Args:
            func: Function to execute (a no-arg lambda/callable)
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Get all edges related to a given node (with retry).
    ...
    def filter_defined_entities(
        ...
        Filtering rules:
        - Skip nodes whose labels are only "Entity"
        - Keep nodes that include labels other than "Entity" and "Node"
        
        Args:
            graph_id: Graph ID
            defined_entity_types: Predefined entity type list (optional)
            enrich_with_edges: Whether to include related edges for each entity
        ...
        # Get all edges (for relationship lookup).
        all_edges = self.get_all_edges(graph_id) if enrich_with_edges else []
        
        # Build node UUID -> node data map.
        node_map = {n["uuid"]: n for n in all_nodes}
        
        # Filter matched entities.
        filtered_entities = []
        ...
        for node in all_nodes:
            ...
            # Labels must include at least one non-default label.
            ...
            # If predefined types are provided, enforce type matching.
            ...
            # Collect related edges and nodes.
    ...
    def get_entity_with_context(
        ...
        try:
            ...
            # Get all nodes for related-node lookup.
            all_nodes = self.get_all_nodes(graph_id)
            node_map = {n["uuid"]: n for n in all_nodes}
