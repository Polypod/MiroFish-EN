...
Zep node/edge list APIs use UUID cursor pagination.
This module wraps automatic pagination (including single-page retries)
and transparently returns complete lists to callers.
"""
...
def _fetch_page_with_retry(
    ...
    **kwargs: Any,
    ...
    """Single-page request with exponential backoff retries on failure.
    Retries only transient network/IO errors.
    """
    if max_retries < 1:
...
def fetch_all_nodes(
    ...
    """Fetch graph nodes with pagination.
    Returns at most max_items items (default: 2000).
    Each page request has built-in retries.
    """
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Fetch all graph edges with pagination and return a complete list.
    Each page request has built-in retries.
    """
    all_edges: list[Any] = []
    cursor: str | None = None
