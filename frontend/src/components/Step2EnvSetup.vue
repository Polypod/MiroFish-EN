    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Use context to automatically call tools, extract entities and relationships from the knowledge graph, initialize simulation individuals, and assign unique behaviors and memories based on real-world seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on simulation needs and real-world seeds, the LLM intelligently sets world time flow, recommendation algorithms, active periods per individual, posting frequency, event triggers, and more
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on narrative direction, automatically generate initial activation events and trending topics to guide the initial state of the simulation world
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation rounds config - shown only after config generation and round calculation are complete -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Round Settings</span>
                <span class="section-desc">MiroFish automatically plans and simulates <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> real-world hours, and each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of real-world time passing</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>If agent count is 100: estimated runtime is about {{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        If agent count is 100: estimated runtime {{ Math.round(autoGeneratedRounds * 0.6) }} minutes
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">If this is your first run, it is strongly recommended to switch to 'Custom Mode' and reduce simulation rounds for a faster preview and lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">Displayed Age in Event</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} years</span>
            </div>
            <div class="info-item">
              <span class="info-label">Displayed Gender in Event</span>
            ...
            <div class="info-item">
              <span class="info-label">Displayed MBTI in Event</span>
...
// Log deduplication: record key info from previous output
...
watch(currentStage, (newStage) => {
  if (newStage === 'Generate Agent Profiles' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Enter config generation stage and start polling config
    if (!configTimer) {
...
// Calculate auto-generated rounds from config (without hardcoded defaults)
...
// Compute total linked topic count across all profiles
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // User selected custom rounds, pass max_rounds param
    params.maxRounds = customMaxRounds.value
    addLog(`Starting simulation, custom rounds: ${customMaxRounds.value} rounds`)
  } else {
    // User chose auto-generated rounds, do not pass max_rounds param
    addLog(`Starting simulation with auto-generated rounds: ${autoGeneratedRounds.value} rounds`)
  }
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      startPolling()
      // Start fetching profiles in realtime
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Update only when API returns valid values to avoid overwriting existing valid data
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Config is being generated, start polling and wait...')
        startConfigPolling()
      }
    }
  } catch (err) {
