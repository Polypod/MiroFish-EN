    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Starting...' : 'Begin Generating Report' }} 
...
const doStartSimulation = async () => {
  ...
  // First reset all state to ensure no interference from previous simulation
  resetAllState()
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Check if simulation is completed (by runner_status or platform completion status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Additional check: If backend hasn't updated runner_status yet, but platforms reported completion
      // Check twitter_completed and reddit_completed status
...
onMounted(() => {
  addLog('Step3 Simulation Runner initialized')
  if (props.simulationId) {
    doStartSimulation()
  }
})
