...
const handleNextStep = (params = {}) => {
  ...
  // Pass custom rounds through query parameters if available
  if (params.maxRounds) {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        // If graceful shutdown error, try force stop
        await forceStopSimulation()
      }
    } else {
      // Environment not running, but process may still be running, check simulation status
    ...
    // Check environment status failure does not affect subsequent process
